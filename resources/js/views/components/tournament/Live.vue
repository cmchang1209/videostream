<template>
    <div class="tournament-live">
        ...
        <el-form :model="form" label-position="top">
            ...
            <el-form-item :label="$store.state.langData.cont.pageFn.table.TournamentGroup" prop="groupId">
        ...
        <el-table :data="battleData" stripe style="width: 100%" :empty-text="$store.state.langData.cont.msg.data.d0001">
            <el-table-column prop="sequence" label="場次" width="60"></el-table-column>
...
export default {
    ...
    methods: {
                ...
                .then(response => {
                    ...
                    if (data.errorCode === 'er0000') {
                        ...
                        if (typeof this.id !== 'undefined') {
                            let fData = this.form.data.filter(iteam => {
                                return iteam.id === this.id
                            })
                            if (fData.length) {
        ...
        handleSelect() {
            ...
            if (this.form.id !== '') {
                axios
                    .get('/api/getTournamentGroupData', {
        ...
        handleGroupSelect() {
            ...
            if (this.form.id !== '' && this.form.groupId !== '') {
                ...
                axios
                    .get('/api/getTournamentBattleData', {
        ...
        search() {
            if (this.form.id !== '' && this.form.groupId !== '') {
                window.location.href = `/tournament/live?id=${this.form.id}&groupId=${this.form.groupId}`
            }
        },
        handleClick(d) {
            this.changeAppLoadingStatus(true)
            axios
                .post('/api/updateTournamentPiData', d)
                .then(response => {
                    let data = response.data.data
                    if (data.errorCode === 'er0000') {
                        this.handleGroupSelect()
        ...
        handleCopyUrl(d) {
            ...
            el.value = `http://${document.location.hostname}/view/tm?id=${d.id}`
